...
Pipeline policy:
- Use task-based model routing (cheap, balanced, high-performance tiers).
- Keep retrieval-first flow; heuristic is only shortlist/fallback support.
- Escalate to higher-tier models only for ambiguous, high-impact decisions.
- Log `model_tier`, token usage, and latency for each LLM-backed step.

Detailed routing thresholds and cost controls are maintained in `implementation-plan.md` (source of truth) to avoid policy duplication across documents.

### 5.6 Orchestrator Failure and Provider Error Handling

`OrchestratorAgent` is responsible for controlled, auditable recovery from provider-side failures.

Required controls:
- bounded retry budget per LLM-backed stage
- stage-aware fallback models
- idempotent retries keyed by the same `request_id`
- circuit breaker for unhealthy model routes
- full retry/fallback trace logging

Detailed failure classes, fallback sequence, and escalation policy are maintained in `implementation-plan.md` (source of truth) to avoid repeated operational policy text.
